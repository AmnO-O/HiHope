...
def apply_lora(model, rank: int = 8, alpha: float = 16.0, dropout: float = 0.0, from_layer: int = 0) -> int:
    """No-op stub: mmBERT uses direct full fine-tuning without parameter-choking LoRA."""
    return 0

def lora_parameters(model) -> list:
    """No-op stub: returns empty list as no LoRA adapters are injected."""
    return []
